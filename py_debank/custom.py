...
def get_balance(address: str, chain: ChainNames or str = '',
    ...
    if chain:
        nfts = nft.collection_list(address=address, chain=chain, proxies=proxies)
        chains.update(nfts)

        tokens = token.balance_list(address=address, chain=chain, raw_data=True, proxies=proxies)
        if tokens:
        ...
        nfts = nft.collection_list(address=address, proxies=proxies)
        chains.update(nfts)

        tokens = token.cache_balance_list(address=address, raw_data=True, proxies=proxies)
