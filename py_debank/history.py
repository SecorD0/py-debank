...
def list_(address: str, chain: ChainNames or str = '', start_time: int or str = 0,
    ...
    if page_count <= 20:
        ...
        response = requests.get(Entrypoints.PUBLIC.HISTORY + 'list', params=params, headers=get_headers(),
...
def token_price(token_id: str, chain: ChainNames or str, time_at: Optional[int or str] = None,
    ...
    response = requests.get(Entrypoints.PUBLIC.HISTORY + 'token_price', params=params, headers=get_headers(),
                            proxies=get_proxy_dict(proxies=proxies))
    json_dict = check_response(response=response)
